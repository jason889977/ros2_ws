...
二维码进入视野后应输出文本；启用相机内参时可通过 `/my_camera/wechat_qr_node/qr_pose` 查看 `PoseStamped`。RViz 图像话题选择 `/my_camera/pylon_ros2_camera_node/image_raw`，QoS 使用 `Best Effort`。停止时只需停止 `basler_camera` 容器。
